...
class InputHandler(ABC):
    ...
    def _generate_files(self, data: list[dict], base_name: str, file_types: list[str]):
        """Helper for creating files dynamically from scripts, now supports transaction folder."""
        try:
            # 🧭 Check if Settings has an active transaction folder (from the first step)
            transaction_dir = getattr(Settings, "TRANSACTION_FOLDER", None)
            if transaction_dir and Path(transaction_dir).exists():
                base_path = Path(transaction_dir) / "script_sequences"
                pipeline_logger.info(f"📂 Using existing transaction folder for script sequences: {base_path}")
            else:
                # 🕓 Fallback: keep old behavior
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                base_path = self.BASE_INPUT_PATH / f"{base_name}_{timestamp}"
                pipeline_logger.info(f"📁 No active transaction folder found. Creating new base path: {base_path}")
            ...
            pipeline_logger.debug(f"generated_files initialized: {generated_files}")
            ...
            for item in data:
                ...
                pipeline_logger.info(f"📁 Created script sequence folder: {folder_path}")
                ...
                for ft in file_types:
                    ...
                    msg = (
                        f"🧮 File: {file_path.name} | Words: {word_count} | Tokens: {token_count}"
                    )
            ...
            pipeline_logger.exception(f"❌ Error while generating script sequence files: {e}")
            raise
