...
class SaverFactory:
    ...
    def save_final_video(video_bytes: bytes, filename: str, save_to: str, db_credentials=None):
        pipeline_logger.info("Entering save_final_video")
        if save_to == "local":
            saver = LocalSaver()
    ...
    def save_all_script_media(video_bytes_list, audio_bytes_list, generated_files):
        pipeline_logger.info("Entering save_all_script_media.")
        """
        ...
        videos_dir = Settings.LOCAL_VIDEO_DIR
        ...
        video_paths = [Settings.LOCAL_VIDEO_DIR]
        audio_paths = [Settings.LOCAL_AUDIO_DIR]
        pipeline_logger.info(f"Video Path:{video_paths}")
        pipeline_logger.info(f"Audio Path:{audio_paths}")
        ...
        print("📁 Video saved to:", video_paths)
        print("📁 Audio saved to:", audio_paths)
