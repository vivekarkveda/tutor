...
class VideoFactory:
    """Factory for generating videos from Manim Python scripts."""

    BASE_OUTPUT_PATH = r"C:\Vivek_Main\Manim_project\Manin_main\src\output\video"

    @staticmethod
    def run_manim_on_files(files, output_folder=None):
        """
        ...
        if output_folder is None:
            output_folder = VideoFactory.BASE_OUTPUT_PATH
        ...
        # After video_bytes_list is populated
        print("🎥 Video bytes list preview (first 5 files):")
        for idx, video_bytes in enumerate(video_bytes_list[:], 1):
            print(f"{idx}: {len(video_bytes)} bytes")
