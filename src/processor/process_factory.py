...
class ProcessFactory:
    ...
    def get_processor(processor_name: str, generated_files):
        if processor_name == "manim":
            ...
            return lambda: PyttsxAudioFactory.text_files_to_audio_bytes(generated_files)
        elif processor_name == "coqui":
            ProcessFactory.process_history.append("coqui")
            return lambda: CoquiAudioFactory.text_files_to_audio_bytes(generated_files)
        elif processor_name == "indic":
            ProcessFactory.process_history.append("indic")
            return lambda: IndicTTSAudioFactory.text_files_to_audio_bytes(generated_files)
        elif processor_name == "kokoro":
            ProcessFactory.process_history.append("kokoro")
            return lambda: KokoroAudioFactory.text_files_to_audio_bytes(generated_files)
        else:
