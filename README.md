...
DRIVE_AUTH_MODE = "service"   # Options: "token" | "service"
DRIVE_CREDENTIALS_PATH = ""   # Path to OAuth client_secret.json (used when auth_mode = "token")
SERVICE_ACCOUNT_PATH = ""     # Path to service_account.json (used when auth_mode = "service")
TOKEN_PATH = ""               # Path to store or load OAuth token.json (used in token mode) - automatic generates token.json
                                just need to provide the path
DRIVE_FOLDER_ID = ""          # Parent Drive folder ID (only used in service mode)
...
This module (`video_pipeline/drive_utils.py`) provides an automated way to upload entire local folders to Google Drive, supporting both **OAuth token-based** and **Service Account-based** authentication modes.

-> Overview:

The upload process takes a local folder (for example, your generated `TRANSACTION_FOLDER`) and mirrors it on Google Drive, preserving the **exact folder hierarchy** — all subfolders and files except __pycache__ are uploaded recursively.

The only difference between the two modes (`token` vs `service`) lies in how Drive authentication is done and where files are uploaded.

While passing `token` we provide the exact folder name or if needed to create a folder and save under it, but in 
`service` we provide the **Folder_ID** of a specific folder where folders and files are needed to be saved.
